...
class PlayerService {
    ...
    static async getPlaylistByDeviceCode(deviceCode) {
      ...
      `)
        ...
        const { data: playlistMedia, error: mediaError } = await supabase_1.supabase
            .from('playlist_media')
            .select(`
        order_index,
        media:media_id (
      ...
      `)
        ...
        const mediaWithUrls = await Promise.all((playlistMedia || []).map(async (item) => {
            const mediaUrl = await media_service_1.MediaService.getMediaUrl(item.media.file_path);
            return {
                ...item.media,
