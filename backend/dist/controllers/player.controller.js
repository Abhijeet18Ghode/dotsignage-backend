...
class PlayerController {
    ...
    static async getPlaylistByDeviceCode(req, res) {
        try {
            ...
            const playlist = await player_service_1.PlayerService.getPlaylistByDeviceCode(deviceCode);
            if (!playlist) {
                res.status(404).json({
            ...
            await player_service_1.PlayerService.updateScreenLastSeen(deviceCode);
            res.json({
                playlist,
                deviceCode,
    ...
    static async getDeviceStatus(req, res) {
        try {
            ...
            const status = await player_service_1.PlayerService.getScreenStatus(deviceCode);
            res.json({
                ...status,
                deviceCode,
                timestamp: new Date().toISOString()
